export function wp(url: string | URL, i: 0 | 1 | 2 | 3 = 0) {
    const u = new URL(url);
...
export function imageCDN(
    ...
    const u = new URL("https://imagecdn.app/v2/image/" + new URL(url).href);
    if (options) Object.entries(options).forEach(([k, v]) => u.searchParams.set(k, String(v)));
    return u.href;
}
